...
(function ($) {
    ...
    stickyHeader: function () {
      ...
      const headerTopWrapper = $('.header-top-one-wrapper');

      function sticky_header() {
        const scroll = $(window).scrollTop();
        const header_height = headerSticky.innerHeight();
        
        // Ensure the header sticks when scrolling down as well
        ...
        if (scroll > lastScroll) {
          headerSticky.addClass('null');
        } else {
          headerSticky.removeClass('back-hide-header');
        }

        headerTopWrapper.toggle(scroll <= 100);
        lastScroll = scroll;
      }
